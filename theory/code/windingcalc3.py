...
SQRT3 = math.sqrt(3)
COPPER_RESISTIVITY_20C = 1.724e-8   # ohm*m, annealed copper
COPPER_TEMP_COEFF = 0.00393         # per C
K_T_CONSTANT = 8.27                 

# Neodymium grade -> Br (tesla), representative midpoint of each grade's
# published tolerance band. Temperature suffix (SH/UH etc) doesn't change
# Br meaningfully - only affects max operating temp / coercivity.
GRADE_LIST = ["N35", "N38", "N40", "N42", "N45", "N48", "N50", "N52"]
GRADE_BR = {
    "N35": 1.195, "N38": 1.235, "N40": 1.265, "N42": 1.30,
    "N45": 1.35, "N48": 1.395, "N50": 1.425, "N52": 1.455,
}
...
# Construction box
CONSTRUCTION_SPECS = [
    ("stator_dia", "stator dia", 20.0, 150.0, 81.0, 0.5),
    ("l_stack", "l_stack", 2, 30, 10.0, 0.5),
    ("w_tooth", "w_tooth", 1, 15, 2.0, 0.1),
    ("r_depth", "r_depth", 2, 30, 8.0, 0.1),
    ("w_slot_in", "w_slot_in", 1, 15, 3.5, 0.1),
    ("w_slot_out", "w_slot_out", 1, 20, 5.0, 0.1),
    ("stator_gap", "stat gap", 0.5, 15.0, 3.5, 0.1),
]
CONSTRUCTION_X = 0.10
...
ax_rotor.set_title("Rotor type (informational)", fontsize=FS_RADIO_TITLE, color=SUBTEXT)
...
ax_k_mode = fig.add_axes([0.5, 0.59, 0.08, 0.06])
...
panel(0.042, 0.69, 0.285, 0.26, "CONSTRUCTION")
...
ax_grade = fig.add_axes([ROTOR_X, ROTOR_GRADE_Y, SLIDER_W, ROTOR_H])
...
ROTOR_SPECS = [
    ...
    ("mu_r_ring", "mu_r ring", 1.0, 2000.0, 2000.0, 1.0),
    ("g", "d_gap (mm)", 0.1, 2.0, 0.65, 0.05),
]
magnet_sliders = build_slider_group(ROTOR_SPECS, ROTOR_X, ROTOR_TOP_Y, ROTOR_STEP, ROTOR_H)

panel(0.042, ROTOR_PANEL_BOTTOM, 0.285, ROTOR_PANEL_HEIGHT, "FLUX CONFIG")

# RESULTS BOX
row4_bottom = 0.267
results_top = row4_bottom - 0.02
...
CORRECTIONS_X = 0.405
CORRECTIONS_TOP_Y = 0.56
CORRECTIONS_STEP = 0.02
CORRECTIONS_H = 0.017
...
def redraw(_=None):
    ...
    mu_r_ring = magnet_sliders["mu_r_ring"].val
    ...
    # Results TEXT
    lines = [
        "Outrunner" if rotor_type == "outrunner" else "Inrunner",
        ...
        f"ring: t={t_ring:.1f}mm mu_r={mu_r_ring:.0f} \u2192 g_backiron={g_backiron:.4f}mm",
